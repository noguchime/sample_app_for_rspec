...
RSpec.describe "Tasks", type: :system do
  ...
  describe 'ログイン後' do
    ...
    describe 'タスク新規登録' do
      context 'フォームの入力値が正常' do
        it 'タスクの新規作成が成功する' do
          ...
          select 'doing', from: 'Status' #selectの選択を指定
      ...
      context '登録済のタイトルを入力' do
        it 'タスクの新規作成が失敗する' do
          visit new_task_path
          fill_in 'Title', with: task.title #同名のタイトルで作成
          fill_in 'Content', with: 'test_content'
          click_button 'Create Task'
          expect(page).to have_content '1 error prohibited this task from being saved'
    ...
    describe 'タスク編集' do
      let!(:task) { create(:task, user: user) } #let!はexampleの実行前にこの処理を行う・user: userによりuser情報がfactorybotのuserと結びつく
      let(:other_task) { create(:task, user: user) }
      before { visit edit_task_path(task) } #exampleの前に必ず実行する
      ...
      # taskを登録したあとother_taskを作成し、そこからtaskを編集してother_taskと同じタイトルにしようとしている
      context '登録済のタイトルを入力' do
        it 'タスクの編集が失敗する' do
